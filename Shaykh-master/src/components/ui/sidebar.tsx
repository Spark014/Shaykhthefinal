...
>(
  (
    ...
    const contextValue = React.useMemo<SidebarContext>(
      ...
      [state, isOpen, handleSetOpen, isMobile, openMobile, toggleSidebar]
    )
...
>(
  (
    ...
    const { isMobile, state, openMobile, setOpenMobile, open: isOpenFromProvider } = useSidebar()
      ...
      return (
        <div
          className={cn(
            ...
            "bg-[hsl(var(--sidebar-background))] text-sidebar-foreground",
            "backdrop-blur-[var(--sidebar-blur)]",
            "backdrop-saturate-[var(--sidebar-backdrop-saturate)]",
            "backdrop-brightness-[var(--sidebar-backdrop-brightness)]",
      ...
      return (
        <RadixSheet open={openMobile} onOpenChange={setOpenMobile}>
          <RadixSheetContent
            side={side}
            className={cn(
              "w-[var(--sidebar-width-mobile)] p-0 flex flex-col",
              "bg-[hsl(var(--sidebar-background))] text-sidebar-foreground",
              "backdrop-blur-[var(--sidebar-blur)]",
              "backdrop-saturate-[var(--sidebar-backdrop-saturate)]",
              "backdrop-brightness-[var(--sidebar-backdrop-brightness)]",
          ...
          >
            {/* Removed custom close button, rely on SheetContent's default close button */}
            <SheetTitle className="sr-only">{mobileSheetTitle}</SheetTitle>
            {/* Only render children except SidebarFooter in scrollable area */}
            <div className="flex-1 overflow-y-auto">
              {React.Children.toArray(children).filter(child =>
                React.isValidElement(child)
                  ? typeof child.type !== "string" && (child.type as { displayName?: string }).displayName !== "SidebarFooter"
                  : true
              )}
            </div>
            {/* Render SidebarFooter only once at the bottom */}
            {React.Children.toArray(children).find(
              (child) =>
                React.isValidElement(child) &&
                (typeof child.type !== "string") &&
                ((child.type as { displayName?: string }).displayName === "SidebarFooter")
            )}
          </RadixSheetContent>
        </RadixSheet>
      )
    ...
    // Always render the desktop sidebar structure
    return (
      <div
        ref={ref}
...
>(({ className, ...props }, ref) => {
  return (
    <main
      ref={ref}
      className={cn(
        "relative flex min-h-svh flex-1 flex-col bg-background", // Main content area uses --background
...
type SidebarMenuButtonCombinedProps = 
...
>(
  (
    ...
    let Comp = asChild ? Slot : isLink ? "a" : "button";
    ...
    if (isLink) {
      ...
      if (typeof componentProps.href !== 'undefined') {
         delete componentProps.href;
      }
    }
    
    const buttonElement = React.createElement(Comp, componentProps, children);
